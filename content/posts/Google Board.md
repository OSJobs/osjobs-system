...
1. 游戏中有数十万甚至更多的玩家
2. 当一个玩家击倒敌人（或者其他行为）的时候，它们的分数会改变
3. 问题是：如何在游戏中显示最新的玩家排名

![Legend of the Criptids, 2012年10月在苹果商店上排名第一]()

![玩家以及它的分数](https://raw.githubusercontent.com/OSJobs/osjobs-system/main/static/Google board/Legend of the Criptids.jpeg)

计算排名很简单，不过扩展性和效率并不高，例如，你可以在数据库运行查询

    SELECT count(key) FROM Players WHERE Score > YourScore 

![玩家排名排序]()

这个查询会返回所有分数玩家大于你的分数的玩家，但是如果你的游戏有一百万的玩家的话，你一定不会想每个玩家在查找排名的时候都像这样查询一次数据库。一开始，Applibot 的工程师 Tomoaki 就是选用了这个方案，但是发现查询花费了几秒钟才得到响应，这个方案实在太慢，太耗费资源了，而且当玩家越多的时候，问题也会更加严重。接着，Tomoaki 想到第二个方案，用 Memcache 维护这些排名数据，这样会快速高效，但是并不可靠，Memcache 存储的对象随时会被删除，事实上，Memcache 本身也不太可靠。一个排名系统仅仅依靠 Memcache，非常难去保证它的一致性与可靠性。最后，Tomoaki 还想到一个临时方案，他决定退而求其次，与其每次请求都计算排名，他使用了一个定时器每个小时定时计算并且更新用户排名。这样的话每次请求都能很快得到结果，但是这个排名可能有一个小时的延迟。
...
1. 初始化，建立一个哈希表，键为玩家 ID，值为分数对象，ID-> 分数对象，时间为 O(n)。接下来，建立一个以（分数对象，ID）为大小的全部玩家数量 n 的二叉平衡树，其中每个节点额外记录左右子树的节点数量。时间为 O(nlog(n))。
2. 要找到某个玩家的排名，我们只需要在哈希表中查找对应 ID 的分数对象的值 ，然后在二叉平衡树中根据其左节点以及父节点的左节点数量递归找到对应排名，时间为 O(log(k))。
3. 要找到某个区间内的排名，例如第 101 到 第 200 名，我们需要查找 100 次对应排名的玩家，时间为 O( k(log(n))，其中 k 为区间的大小。
4. 更新玩家分数：更新哈希表，并且更新二叉平衡树的节点，时间为 O(log(K))。
...
- 前端：接受SetScore请求，并把任务放到队列
- 队列：持续接受保存前端发过来的SetScore请求 
...
1. 把SetScore任务放到 10个 队列中
2. 使用另外一个方法 SetScores从队列中获取任务并且合并，分发给后端 
3. 从队列中删除执行成功的任务
...
1. 把现有数据分成 1000 个窗口，每个窗口大小为 100。
...
- SetScore ：当需要更新玩家分数的时候，例如把用户 A 更新为 30 分。先从哈希表 T 中找到用户 A 的旧分数，更新 哈希表 T 中用户 A 的分数为 30 分，并记录旧分数。，从 V 中把旧分数对应的频率减 1（如果没出现就不用减），把 30 对应的频率加 1。
